...
function getData() {
  return categoryStore
    .getCategoryGoodsAction(category_id.value)
    .then((res: any) => {
...
const handleDelete = (row: any) => {
  row.loading = true
  categoryStore.deleteCategoryGoodsAction(row.id).then(res => {
    toast('删除成功')
    getData()
  })
}

const ChooseGoodsRef = ref<any>(null)
const handleConnect = () => {
  ChooseGoodsRef.value.open((goods_ids: any) => {
    formDrawerRef.value.showLoading()
    categoryStore
      .connectCategoryGoodsAction({
