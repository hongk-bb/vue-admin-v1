...
const {
  ...
  getList: roleStore.getRoleListAction,
  delete: roleStore.deleteRoleAction,
  updateStatus: roleStore.updateRoleStatusAction
})
...
const {
  ...
  getData,
  update: roleStore.updateRoleAction,
  create: roleStore.createRoleAction
})

const setRuleFormDrawerRef = ref<any>(null)
const ruleList = ref([])
...
const checkStrictly = ref<any>(false)

const openSetRule = (row: any) => {
  roleId.value = row.id
  treeHeight.value = window.innerHeight - 180
  checkStrictly.value = ruleStore.getRuleListAction(1).then(res => {
...
const handleSetRuleSubmit = () => {
  ...
  roleStore
    .setRoleRulesAction(roleId.value, ruleIds.value)
    .then(res => {
      toast('配置成功')
      getData()
