...
import { useGoodsCommentStore } from '@/stores/goods_comment'
...
const {
  ...
  getList: goodsCommentStore.getGoodsCommentListAction,
  ...
  updateStatus: goodsCommentStore.updateGoodsCommentStatusAction
})

const textarea = ref('')
const openTextarea = (row: any, data: any = '') => {
  textarea.value = data
...
const review = (row: any) => {
  ...
  goodsCommentStore
    .reviewGoodsCommentAction(row.id, textarea.value)
    .then(res => {
